...
const getProvider = (chainId, providerUrl) => {
    ...
    if (chainId === constants_1.SupportedChainId.Arbitrum) {
        // Create a static network to bypass detection issues
        const network = new ethers_1.ethers.Network("arbitrum", 42161n);
        return new ethers_1.ethers.JsonRpcProvider("https://obsidian-rpc-v2.up.railway.app/rpc", network, { staticNetwork: network });
    }
